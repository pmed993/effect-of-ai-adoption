...
# ---- Cleaning ----
extract_documents_from_submission <- function(raw_lines) {
  x <- paste(raw_lines, collapse = "\n")
  # split by <DOCUMENT> blocks
  docs <- stringr::str_split(x, "(?i)</DOCUMENT>")[[1]]
  docs <- docs[stringr::str_detect(docs, "(?i)<DOCUMENT>")]
  
  parse_one <- function(d) {
    # pull TYPE: and TEXT:
    type <- stringr::str_match(d, "(?i)\\nTYPE:\\s*([^\\n]+)")[,2]
    # content after <TEXT>
    txt  <- stringr::str_split(d, "(?i)<TEXT>")[[1]]
    body <- if (length(txt) >= 2) txt[2] else d
    list(type = stringr::str_trim(type), body = body)
  }
  
  out <- lapply(docs, parse_one)
  # return data.table
  data.table(
    type = sapply(out, `[[`, "type"),
    body = sapply(out, `[[`, "body")
  )
}

choose_main_10k_doc <- function(docs_dt) {
  # prefer TYPE that is 10-K or variants
  # you can expand to include 10-K405, 10KSB, etc if needed
  keep <- docs_dt[stringr::str_detect(type, "(?i)^10-k")]
  if (nrow(keep) == 0) return(NA_character_)
  
  # pick the longest body (main doc tends to be longest)
  keep[which.max(nchar(body)), body]
}
...
docs <- extract_documents_from_submission(raw)
main <- choose_main_10k_doc(docs)

if (is.na(main)) { item1 <- NA; item7 <- NA } else {
  clean <- clean_10k_text(strsplit(main, "\n", fixed = TRUE)[[1]])
  item1 <- extract_item_section(clean, 1)$text
  item7 <- extract_item_section(clean, 7)$text
  if (is.na(item7)) item7 <- extract_mda_fallback(clean)$text
}
...
clean_10k_text <- function(raw_lines) {
  ...
  text <- gsub("&amp;", "&", text, ignore.case = TRUE)
  text <- gsub("&quot;|&#34;|&#8220;|&#8221;|&ldquo;|&rdquo;", "\"", text, ignore.case = TRUE)
  text <- gsub("&#8216;|&#8217;|&lsquo;|&rsquo;|&#39;", "'", text, ignore.case = TRUE)
  text <- gsub("&lt;", "<", text, ignore.case = TRUE)
  text <- gsub("&gt;", ">", text, ignore.case = TRUE)
...
LLM_SLEEP <- 2.0         # base delay between LLM calls
...
RESET_CACHE <- TRUE
if (RESET_CACHE) {
  unlink("cache/sections", recursive = TRUE)
  dir.create("cache/sections", showWarnings = FALSE, recursive = TRUE)
}
...
# ---- Cache sections to disk (Level 1 cache) ----
unlink("cache/sections", recursive = TRUE) ###--------------------------------------------------------------- Cleaning cache
dir.create("cache/sections", showWarnings = FALSE)
